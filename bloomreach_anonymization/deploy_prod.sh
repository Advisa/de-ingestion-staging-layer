...
gcloud functions deploy bloomreach_anonymization \
    --runtime python310 \
    --trigger-http \
    --region europe-west1 \
...
gcloud scheduler jobs update http bloomreach_anonymize_prod_trigger \
  --location "europe-west1" \
  --description "Triggers the prod anonymisation service" \
  --schedule "5 1 * * *" \
  --uri "https://europe-west1-sambla-data-staging-compliance.cloudfunctions.net/bloomreach_anonymization" \
  --http-method POST \
